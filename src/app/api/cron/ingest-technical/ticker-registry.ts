 ...
 *
 * Order is stable (KR → US → region ETF → macro-hedge ETF) because the
 * cron walks this list sequentially with a 13-second sleep between
 ...
 * affect correctness, but the deterministic walk helps when debugging
 * a partial run against `ingest_runs.duration_ms`.
 */
export const TICKER_REGISTRY: readonly TickerRegistryEntry[] = [
  // ----- 7 KR equities / KR ETFs -----
 ...
 * design note 1). 19 tickers × 13s ≈ 247s, within Vercel Fluid Compute
 * default max 300s.
 *
