...
import { ModelVersionBadge } from "@/components/ui/model-version-badge";
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 md:space-y-8">
      <div className="flex items-start justify-between gap-4">
        ...
        {/*
          Model-version badge (blueprint §4.4 Step 3). Wrapped in its
          own `<Suspense>` boundary because `ModelVersionBadge` is an
          async Server Component that reads the `'use cache'` scope in
          `src/lib/data/model-version.ts`. Under `cacheComponents:
          true` a cached reader needs a Suspense parent or the whole
          static shell bails to dynamic — keeping the boundary local
          lets the hero text + blurb prerender even if the badge has
          to fetch fresh.
        */}
        <Suspense
          fallback={
