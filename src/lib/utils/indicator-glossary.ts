...
export const INDICATOR_GLOSSARY: Record<string, IndicatorGlossaryEntry> = {
  ...
  STLFSI4: {
    key: "STLFSI4",
    labelKo: "금융스트레스지수 (STLFSI)",
    sourceName: "FRED",
    sourceUrl: "https://fred.stlouisfed.org/series/STLFSI4",
    category: "macro",
    shortKo: "금리·신용·주식 18개 지표를 종합한 금융시스템 스트레스 온도계",
    beginnerExplanationKo:
      "세인트루이스 연준이 금리, 신용 스프레드, 주식 변동성 등 18개 지표를 하나로 합쳐 만든 지수입니다. 0이 역사적 평균이며, 숫자가 클수록 금융시장 전반에 긴장이 높다는 뜻입니다. 개별 지표가 놓칠 수 있는 시스템 전체의 스트레스를 한 번에 보여줍니다.",
    bullishCaseKo:
      "0 아래로 내려가 있으면 평균보다 안정적인 상태로, 조정이 오더라도 시스템 위기로 번질 가능성이 낮아 '할인 구간' 판단에 우호적인 근거가 됩니다.",
    bearishCaseKo:
      "1을 넘으면 금융시장 전반의 긴장 신호이며, 3 이상은 2008년·2020년급 위기 수준입니다. 이때의 하락은 단순 조정이 아닐 확률이 높아 방어가 우선입니다.",
    typicalRangeKo:
      "평상시는 -1 ~ 0 부근이며, 0~1은 관찰 구간, 1 이상 스트레스 고조, 3 이상은 위기로 분류됩니다.",
    caveatKo:
      "주간 발표라 일중 급변은 반영이 하루 이틀 늦을 수 있습니다. 빠른 신호는 VIX·하이일드 스프레드가 먼저 움직입니다.",
    scoreDirectionKo:
      "이 지표는 합성 점수에는 들어가지 않고 어드바이저의 매크로 근거 기둥에만 반영됩니다. 값이 낮을수록(음수) 할인 근거, 높을수록 전환 경고 근거로 작동합니다.",
    scoringMethodKo:
      "0-100 점수를 만들지 않는 raw 지표입니다. 어드바이저가 0(역사 평균)과 1(스트레스), 3(위기) 기준선으로 구간을 나눠 근거 문장에 반영합니다.",
    unitKo: "σ",
  },
};
