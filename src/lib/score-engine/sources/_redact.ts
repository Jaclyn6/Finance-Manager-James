...
/**
 * Return a copy of `message` with any `apikey=...` or `token=...`
 * query-string values replaced by `REDACTED`. Idempotent; safe to
 * call on already-redacted strings.
 */
export function redactSecretsFromErrorMessage(message: string): string {
  return message.replace(SECRET_QUERY_PARAM_PATTERN, "$1$2=REDACTED");
}
