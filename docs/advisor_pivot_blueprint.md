...
| Series readers page past PostgREST max-rows (1,000) — the 5y percentile window was silently truncated to its OLDEST rows (VIX "5년 하위 1%" for a real 42nd-percentile value); per-key `.range()` paging with id tiebreak, coverage-aware window labels ("3년" for the younger HY series) | 99d7935+ | 2026-08-03 |
| SIGNAL_RULES_VERSION v1.1.0 — EXTREME_FEAR arm 2 falls back to `STOCK_FG_PROXY` when CNN_FG is null (CNN 418-blocked since 06-24 left the tile permanently 판단 보류; user green-lit substitution). CNN-first self-healing, 자체 프록시 labeling in tile copy, `loadSignalInputs` per-key versioned proxy read | (this commit)+ | 2026-08-03 |
