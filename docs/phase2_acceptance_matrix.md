...
|---|---|---|
| RSI, MACD, 이동평균선 반영 (line 734) | Pure-math module `src/lib/score-engine/technical.ts` covered by `src/lib/score-engine/technical.test.ts` (62 `it/test` blocks: RSI thresholds, MACD cross, MA_50, BB_20_2, Disparity). Daily cron `src/app/api/cron/ingest-technical/route.ts` writes `technical_readings` for the **19-ticker registry post-Phase-3.0** (12 US/global via AV→Twelve Data fallback + 7 KR via Yahoo Finance). All 6 indicators populate, including MA_200 + Disparity (Twelve Data 300-bar window). 2026-04-26 manual dispatch verified via Supabase: 18/18 MA_200 non-null, 42/42 KR rows non-null with `source_name='yahoo_finance'`. | MET |
| BTC MVRV / SOPR 반영 (line 735) | `src/lib/score-engine/onchain.ts` + `src/lib/score-engine/onchain.test.ts` (44 test blocks). **Phase 3.0 cron-onchain split** (`cron-onchain.yml`, every 4h): 6 fires/day × 2 endpoints = 12 BGeometrics calls/day, safely under the documented 15/day cap (3 calls headroom for `workflow_dispatch`). MVRV_Z / SOPR drive `CRYPTO_UNDERVALUED` / `CAPITULATION` in `signals.ts`. The 8/hr-vs-15/day resilience gap that produced intermittent 429s in Phase 2 is closed. | MET |
...
| PWA 대응 — §11.7 (line 740) | `public/manifest.webmanifest` (PNG + SVG icons, both standard and maskable purpose, `start_url=/dashboard`, `display=standalone`). `public/sw.js` shell-only (`finance-shell-v2`, network-first, no API caching per blueprint §6.2). Registration via `src/components/shared/service-worker-registration.tsx`. **2026-04-26 production audit (Lighthouse 13.1.0, mobile, deployment `2058bd9` aliased to `finance-manager-james.vercel.app`, audited URL `/login?next=%2Fdashboard` per the auth gate)**: Performance 94 / Accessibility 95 / Best Practices 100 / SEO 100. Manifest HTTP 200 with all 6 icon variants (192/512 PNG, 192/512 SVG, maskable 512 PNG+SVG) returning HTTP 200. `sw.js` HTTP 200, 3,118 bytes. Lighthouse 13 deprecated the `pwa` category — installability is now adjudicated via individual audits + manual checks. Real-device A2HS smoke test on iOS Safari + Android Chrome remains pending. | PARTIAL |
| 매수 타이밍 시그널 엔진 — §10.4 (line 741) | `src/lib/score-engine/signals.ts` (8 signals: 6 base + `CRYPTO_UNDERVALUED` + `CAPITULATION`); `signals.test.ts` has 54 `it/test` blocks (well above the blueprint's "18+" floor). `src/lib/data/signals.ts` writes `signal_events` keyed by `(snapshot_date, signal_rules_version)` — `SIGNAL_RULES_VERSION='v1.0.0'` from `src/lib/score-engine/weights.ts:46`. `SignalAlignmentCard` rendered on all 3 protected routes — `/dashboard` (above `CompositeStateCard`), `/asset/[slug]`, and `/changelog` (added in commit `8261924`). | MET |
...
| Criterion | Evidence | Status |
|---|---|---|
| Cron reliability — 7 consecutive days green (line 753) | GHA workflows `.github/workflows/cron-hourly.yml` (onchain + cnn-fg + news, hourly) and `.github/workflows/cron-technical.yml` (technical + prices, daily 22:00 UTC). `continue-on-error: true` per blueprint §0.5 tenet 1; truth source is `ingest_runs` rows, not GHA badge color. **2026-04-26 day-1 status (UTC 20:01, KST 05:01)**: cron-hourly 8/8 most-recent runs SUCCESS through 2026-04-25 19:43 UTC (durations 1m12s–1m22s). cron-technical: 5 prior runs all FAILURE but ALL predate fix commits — last scheduled fire at 2026-04-24 22:34Z (failed in 6s, before PRODUCTION_URL/CRON_SECRET sync), last manual dispatch 2026-04-25 02:57Z (4m18s timeout, before single-batch revert + KR carve-out at `d536141`). The first scheduled fire under the fixed config will be 2026-04-25 22:00 UTC (KST 2026-04-26 07:00) — to be re-checked by the next session. Tracking window: 2026-04-25 → 2026-05-02. | PARTIAL |
...
- **MET:** 13 / 16 = 81% (incremented 2026-04-26 by Phase 3.0: §10.1 row 1 "RSI/MACD/MA coverage" promoted PARTIAL → MET after Twelve Data + Yahoo fallback chain restored MA_200 + Disparity; §10.1 row 2 "BTC MVRV/SOPR" promoted PARTIAL → MET after `cron-onchain.yml` 4h split closed the 15/day BGeometrics cap. Earlier today: §10.1 row 8 "시그널 엔진" + §10.3 row 3 "SignalCard on /changelog" both promoted via commit `8261924`.).
- **PARTIAL:** 3 / 16 = 19% (`/changelog` cron 7-day green window, PWA real-device A2HS, price-readings ESLint guard).
- **DEFERRED:** 0 / 16 = 0%.

The 3 remaining PARTIAL rows are operational tail items: (1) cron 7-day green watch (Day 1 = 2026-04-26 post-Phase-3.0; matures 2026-05-03), (2) real-device A2HS smoke test (user-side), (3) ESLint `no-restricted-imports` rule for `data/prices` is convention-only — not landed yet but the blueprint §7.4 invariant is grep-verified clean. None blocks Phase 3.x.
